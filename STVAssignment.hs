-- Function Programming Assignment : Voting System
-- 
--
--
-- Rhianna Paine
...
ballots :: [[String]]
ballots = [["","","D. Abbott","E. Balls","A. Burbhm","D. Milliband","E. Milliband"],
    ["1","Ms D Abbott MP  ","1","*","*","*","*"],
...
-- Remove ballots with empty strings (Page Breaks)
rmemptystrings :: [[String]] -> [[String]]
rmemptystrings = filter (not . null) . map (filter (not . null)) 

-- Remove vote number and names (first 2 elements)
rmnameandno :: [[String]] -> [[String]]
rmnameandno (x:xs) = [drop 2 x | x <- xs]


-- Number of clean votes

getcleanvotes ::  [[String]] 
getcleanvotes = rmnameandno (rmemptystrings ballots)

countcleanvotes :: Int
countcleanvotes = length getcleanvotes
...
quota :: Int --may need to change number of seats
quota = ((countcleanvotes * weight) `div` numofseats) + 1


-- Count all first nominee firsts

--Get all first votes in ballot
--groupballot :: [[String]] -> [[String]]
groupballot = transpose getcleanvotes
